...
- [x] ログインを任意にして未ログインでも投稿・閲覧できるようにする
- [x] ログイン画面を投稿後のオプションプロンプトとして表示（投稿画面の後ろに移動）
- [x] マイページ（ProfilePage）が真っ白になるバグを修正
- [x] ログイン済み時のProfilePage白画面を調査・修正
- [x] i18n（国際化）: ブラウザ言語設定に応じて日本語/英語を自動切替（react-i18next）
- [x] 翻訳ファイル作成（ja.json / en.json）
- [x] 全ページ・コンポーネントのUI文字列を翻訳キーに置換
- [x] ヘッダーに🌐言語切り替えボタンを追加（日本語/英語の手動切替）
- [x] 言語切り替えボタンをPostPage・ProfilePage・SearchPageのヘッダーにも追加
